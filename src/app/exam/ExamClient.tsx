...
export default function ExamClient({ searchParams }: ExamClientProps) {
  ...
  useEffect(() => {
    const slug = searchParams.exam;
    if (slug) {
      const foundExam = examData.find((exam) => slugify(exam.name) === slug);
      if (foundExam) setSelectedExam(foundExam);
  ...
  const handleSelectExam = useCallback(
    (exam: ExamData) => {
      ...
      router.replace(`?exam=${slugify(exam.name)}`);
    },
    [router]
  );
  ...
  const handleViewDetails = useCallback(
    (exam: ExamData) => handleSelectExam(exam),
    [handleSelectExam]
  );
              ...
              <div className="p-4 md:p-8 space-y-6">
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 md:gap-8">
                  ...
                  <div className="flex items-center gap-3 ml-auto">
                    <Button variant="ghost" onClick={handleCloseExam}>
                      Close
                    </Button>
                    <a
                ...
                {/* Sections: Overview, Eligibility, Syllabus, Dates, Cutoff */}
